...
function portalsluchu_kasa_render_checkout_section( $checkout = null ) {
    ...
    <div class="portalsluchu-kasa-box" style="margin-top:20px; padding:15px; border:1px solid #ddd;">
        ...
        <div id="portalsluchu_delivery_dojazd_box" style="margin-left:15px; margin-bottom:10px; <?php echo ( $delivery_method === 'dojazd' ) ? '' : 'display:none;'; ?>">
            ...
            <p style="color: #d63638; font-size: 0.9em; margin-top: 0;">
        ...
        <div id="portalsluchu_delivery_salon_box" style="margin-left:15px; margin-bottom:10px; <?php echo ( $delivery_method === 'salon' ) ? '' : 'display:none;'; ?>">
            <p>
                <select name="portalsluchu_delivery_salon" style="max-width:320px;">
                    ...
                    <option value="Poznań – Grunwald" <?php selected( $delivery_salon, 'Poznań – Grunwald' ); ?>>Poznań – Grunwald</option>
                    <option value="Wrocław – Fabryczna" <?php selected( $delivery_salon, 'Wrocław – Fabryczna' ); ?>>Wrocław – Fabryczna</option>
                    <option value="Katowice – Południe" <?php selected( $delivery_salon, 'Katowice – Południe' ); ?>>Katowice – Południe</option>
                    <option value="Warszawa – Mokotów" <?php selected( $delivery_salon, 'Warszawa – Mokotów' ); ?>>Warszawa – Mokotów</option>
                    <option value="Szczecin – Centrum" <?php selected( $delivery_salon, 'Szczecin – Centrum' ); ?>>Szczecin – Centrum</option>
                </select>
            </p>
            <p style="color: #d63638; font-size: 0.9em; margin-top: 5px;">
                Po złożeniu zamówienia skontaktujemy się z Tobą mailowo, aby umówić dokładny termin wizyty oraz podać pełny adres wybranego salonu.
            </p>
        </div>
    ...
    <script>
    (function($) {
        function refreshBoxes(animate) {
            var val = $('input[name="portalsluchu_delivery_method"]:checked').val();
            if (!val) val = 'salon';
            
            var $salonBox  = $('#portalsluchu_delivery_salon_box');
            ...
            if (animate) {
                ...
                if (val === 'salon') {
                    ...
                    if ($salonBox.is(':visible')) $salonBox.slideUp(600);
                }
                
                if (val === 'dojazd') {
                    if ($dojazdBox.is(':hidden')) $dojazdBox.slideDown(600);
                } else {
        ...
        $(document).on('change', 'input[name="portalsluchu_delivery_method"]', function() {
            refreshBoxes(true);
 ...
 * - dodano obsługę maski kodu pocztowego (00-000) - bezpieczne dla PL.
 *
    ...
    (function($){
        ...
        function reorderBillingFields(){
            ...
            var ordered = [
                ...
                $invoiceField // Chcę FV na końcu standardowej listy
            ];
            ...
            // --- SEKCJA PŁYNNEGO POJAWIANIA SIĘ FIRMY/NIP ---
            
            // Tworzymy kontener (wrapper) na pola firmowe, jeśli nie istnieje
            var $companyWrapper = $('#ps-company-nip-wrapper');
            if (!$companyWrapper.length) {
                // Początkowo ukryty
                $companyWrapper = $('<div id="ps-company-nip-wrapper" style="display:none;"></div>');
                // Wstawiamy go w DOM tuż PO polu "Chcę FV"
                if ($invoiceField.length) {
                    $invoiceField.after($companyWrapper);
                } else {
                    // Fallback jeśli nie ma pola FV
                    $wrap.append($companyWrapper);
                }
            } else {
                // Jeśli istnieje (po odświeżeniu checkoutu), upewnij się, że jest we właściwym miejscu
                if ($invoiceField.length) {
                    $invoiceField.after($companyWrapper);
                }
            }

            // Przenosimy pola do wrappera
            if ($company.length) $companyWrapper.append($company);
            if ($nip.length)     $companyWrapper.append($nip);

            // Ustawiamy widoczność na start (bez animacji przy ładowaniu)
            toggleInvoiceFields(false);
        }

        function toggleInvoiceFields(animate) {
            var $checkbox = $('#portalsluchu_want_invoice');
            var $wrapper  = $('#ps-company-nip-wrapper');
            ...
            if (animate) {
                if (isChecked) {
                    if ($wrapper.is(':hidden')) $wrapper.slideDown(600);
                } else {
                    if ($wrapper.is(':visible')) $wrapper.slideUp(600);
                }
            } else {
                if (isChecked) $wrapper.show(); else $wrapper.hide();
            }
        }
        ...
        // Po każdym odświeżeniu AJAX checkoutu (np. zmiana wysyłki) trzeba przywrócić porządek
        $(document).on('updated_checkout', function(){
            reorderBillingFields();
        });

        // Obsługa kliknięcia "Chcę FV" - animacja
        $(document).on('change', '#portalsluchu_want_invoice', function(){
            toggleInvoiceFields(true);
        });

        // Obsługa maski kodu pocztowego (tylko cyfry, auto-myślnik)
        $(document).on('input', '#billing_postcode, #shipping_postcode', function() {
            ...
            // 2. Ogranicz do 5 cyfr (format PL)
            if (digits.length > 5) {
                digits = digits.substring(0, 5);
            }
            
            // 3. Zbuduj format XX-XXX
            var formatted = digits;
            if (digits.length > 2) {
                formatted = digits.substring(0, 2) + '-' + digits.substring(2);
            }
            
            // 4. Nadpisz wartość tylko jeśli się różni
